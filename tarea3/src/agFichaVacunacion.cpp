...
bool igualesTAGFichaVacunacion(TAGFichaVacunacion fv1, TAGFichaVacunacion fv2){
    ...
    if(!existeVacunaTAGFichaVacunacion(fv2, fv1->codigo)) return false;
    if(!existeVacunaTAGFichaVacunacion(fv1, fv2->codigo)) return false;
    if(fv1->codigo != fv2->codigo){
        return true;
    }
    return (igualesTAGFichaVacunacion(fv1->primerHijo,fv2->primerHijo) && igualesTAGFichaVacunacion(fv1->segundoHermano, fv2->segundoHermano));
}
